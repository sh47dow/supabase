...
const handlePost = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        ...
        if (e.code === 120029) {
            return res.status(401).json({ error: { message: '应用升级中，请稍候'} })
        } else {
            return res.status(401).json({ error: { message: e.msg || 'unknown error' } })
        }
    }
}
...
const handlePatch = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        ...
        if (e.code === 120029) {
            ...
            return res.status(401).json({ error: { message: e.message || 'unknown error' } })
        }
    }
}
